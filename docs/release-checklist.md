...
## Candidate AI Planning Smoke

> Historical note: This candidate smoke contract was formalized during the v1.18.0 candidate cycle and remains the standard release acceptance contract for subsequent release candidates.
...
- 设置页显示与 `package.json.version` / candidate tag 一致的当前版本（`v${package.json.version}`）和本版本内置更新摘要；
