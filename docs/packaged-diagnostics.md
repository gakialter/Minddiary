...
Portable and Setup smoke prove only the CI-built candidates and exact workflow head. They are not clean-host browser-download evidence, updater download/install/restart evidence, a signature or SmartScreen reputation claim, or proof about already-published assets. The macOS DMG/ZIP user flow, notarization, Gatekeeper, and physical Apple-silicon acceptance remain separate gates.

Windows CI separately runs an installed NSIS updater E2E. Two detached disposable worktrees build candidates from the same reviewed head: the old candidate keeps the committed package version, while the new worktree alone advances to the next semantic patch. The installed old candidate receives an electron-builder-generated generic `app-update.yml` targeting a reserved random IPv4-loopback port. A test-only NSIS include in those worktrees gives the installer-created launch shortcut a disposable `--user-data-dir`; this is required because electron-builder's `ExecShellAsUser` restart deliberately uses the desktop user's environment rather than the parent app's overridden `APPDATA`, and it prevents the auto-restarted test app from touching a real default profile. The include is never written to the main worktree or production package configuration. A fixed-allowlist server supplies generated channel metadata, installers, and blockmaps for no-update, malformed-metadata, bad-checksum, and positive cases and records fixed negative probes for method, Host, credentials, cookie, query, traversal, directory, and allowlist enforcement. The only accepted query is `electron-updater`'s generated `latest.yml?noCache=<base32 timestamp>` cache buster; every other query shape remains rejected and the evidence records that distinction without preserving the value. The positive case is driven through the real Settings UI, typed preload/IPC, `electron-updater`, downloaded-only install gate, silent NSIS `quitAndInstall`, and the restarted executable from the physical install directory. The existing token-bound `install-profile` diagnostic seeds fixed fake entry/attachment data before the update and reopens the same profile with the updated installed executable to verify application/runtime version, Electron ABI, SQLite schema, data retention, `local://` loading, and cleanup.

Successful updater evidence and failed-run diagnostics use separate fixed-schema artifacts, are bound to the exact head, and include SHA-256 manifests. They exclude worktrees, installers, database rows, attachment contents, tokens, paths, environment variables, request bodies, credentials, raw errors, and process output. Failed diagnostics retain a safe primary phase, bounded cleanup failure labels, and resource-removal booleans without being accepted as success evidence. The server binds only `127.0.0.1`; evidence proves updater requests observed by this local provider, not that the runner made no unrelated network requests. The gate does not prove production signing, SmartScreen, the GitHub-hosted release channel, already-published assets, Portable updates, or macOS updates.
