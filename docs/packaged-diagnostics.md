...
Windows CI separately runs an installed NSIS updater E2E. Two detached disposable worktrees build temporary `1.16.0` and `1.16.1` candidates from the same reviewed head; only the temporary new worktree changes package metadata. The installed old candidate receives an electron-builder-generated generic `app-update.yml` targeting a random IPv4 loopback port. A fixed-allowlist server supplies generated channel metadata, installers, and blockmaps for no-update, malformed-metadata, bad-checksum, and positive cases. The positive case is driven through the real Settings UI, typed preload/IPC, `electron-updater`, downloaded-only install gate, and `quitAndInstall` path. The existing token-bound `install-profile` diagnostic seeds fixed fake entry/attachment data before the update and reopens the same profile with the updated installed executable to verify application/runtime version, Electron ABI, SQLite schema, data retention, `local://` loading, and cleanup.

The updater evidence artifact contains only fixed-schema JSON and hashes from an exact allowlist. It excludes worktrees, installers, database rows, attachment contents, tokens, paths, environment variables, request bodies, credentials, raw errors, and process output. The server binds only `127.0.0.1`, rejects non-GET/HEAD methods, unexpected Host values, traversal, directories, credentials, cookies, and non-allowlisted resources. Evidence proves updater requests observed by the local provider; it does not claim the entire runner made no unrelated network requests. It also does not prove production signing, SmartScreen, the GitHub-hosted release channel, already-published assets, Portable updates, or macOS updates.
