...
| `test:e2e:setup-smoke` | Runs the actual root NSIS Setup candidate on a disposable Windows runner, verifies silent custom-path install, files, shortcuts, uninstall registration, installed packaged diagnostics, a token-bound diagnostic profile through uninstall/reinstall, final cleanup, no remaining process, and bounded archived evidence; the default-data retention policy is separately locked by `deleteAppDataOnUninstall: false` | Dynamic deletion behavior against a real default user profile, clean-host browser download, updater install/restart, publisher identity, SmartScreen reputation, or the already-published Setup asset |
