...
- 一句话复盘会追加到今日日记；如果当天还没有日记，用户确认后会创建今日日记。
...
- 今日闭环指标：展示今日任务完成率、任务专注覆盖率、任务专注分钟和未闭环任务提示；`skipped` 不进入完成率分母。
...
- 数据库使用显式 schema version 和安全 migration；v1.10.0 升级到 schema version 3，`pomodoro_sessions.task_id` 支持任务归因并在任务删除后自动置空。
