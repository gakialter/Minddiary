...
- 当前正式应用基线为 MindDiary v1.13.0，SQLite schema version 为 5；数据库使用显式 schema version 和安全 migration，并继续兼容受支持的旧数据库升级路径。
...
- 当前 schema 5 自动备份包含详细章节和 `study_tasks.related_chapter_id`；恢复旧 schema 4 备份时，缺失的章节任务归因安全恢复为 `null`，旧 schema 3 备份没有章节表时仍恢复为原有汇总模式。
