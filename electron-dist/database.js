...
function initialize() {
  ...
  `);
    ...
    try {
        db.exec('CREATE INDEX IF NOT EXISTS idx_mistakes_next_review ON mistakes(next_review_date)');
    }
    catch { /* ignore */ }
}
// ==================== Entries ====================
function createEntry({ date, title, content, mood }) {
    ...
    const stmt = db.prepare('INSERT INTO entries (date, title, content, mood, word_count) VALUES (?, ?, ?, ?, ?)');
...
function updateEntry(id, { title, content, mood }) {
    const wordCount = (content || '').replace(/\s/g, '').length;
    const stmt = db.prepare('UPDATE entries SET title=?, content=?, mood=?, word_count=?, updated_at=CURRENT_TIMESTAMP WHERE id=?');
...
function searchEntries(query) {
    ...
    return db.prepare('SELECT id, date, title, mood, word_count, created_at, updated_at, SUBSTR(content, 1, 200) AS content_snippet FROM entries WHERE content LIKE ? OR title LIKE ? ORDER BY date DESC').all(searchTerm, searchTerm);
}
function getDatesWithEntries(yearMonth) {
    const pattern = `${yearMonth}%`;
    return db.prepare('SELECT date, mood FROM entries WHERE date LIKE ?').all(pattern);
}
// ==================== Tags ====================
function getAllTags() {
...
function getEntryTags(entryId) {
    return db.prepare('SELECT t.* FROM tags t JOIN entry_tags et ON t.id = et.tag_id WHERE et.entry_id = ?').all(entryId);
}
// ==================== Settings ====================
...
function addAttachment(entryId, { filename, filepath, mimetype }) {
    const stmt = db.prepare('INSERT INTO attachments (entry_id, filename, filepath, mimetype) VALUES (?, ?, ?, ?)');
...
function createSubject({ name, total_chapters, color }) {
    const stmt = db.prepare('INSERT INTO subjects (name, total_chapters, color) VALUES (?, ?, ?)');
...
function updateSubject(id, { name, total_chapters, completed_chapters, color }) {
    db.prepare('UPDATE subjects SET name=?, total_chapters=?, completed_chapters=?, color=? WHERE id=?').run(name, total_chapters, completed_chapters, color, id);
...
function addPomodoroSession({ subject_id, duration }) {
    const stmt = db.prepare('INSERT INTO pomodoro_sessions (subject_id, duration) VALUES (?, ?)');
...
function getDailyStudyMinutes(date) {
    const row = db.prepare('SELECT COALESCE(SUM(duration), 0) as total FROM pomodoro_sessions WHERE DATE(completed_at) = ?').get(date);
    return row.total;
}
...
function getTodayDashboard(date) {
    ...
    const getTodayData = db.transaction(() => {
        ...
        const todayEntry = db.prepare('SELECT id, title, word_count, mood FROM entries WHERE date = ?').get(date) || null;
...
function createMistake({ subject_id, question, answer, notes, image_path }) {
    const stmt = db.prepare('INSERT INTO mistakes (subject_id, question, answer, notes, image_path) VALUES (?, ?, ?, ?, ?)');
...
function deleteMistake(id) {
    try {
        ...
        console.error('Failed to cleanup mistake image', e);
    }
