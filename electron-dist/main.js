...
ipcMain.handle('updater:check', async () => {
    if (!autoUpdater)
        return { success: false, message: '环境不支持自动更新' };
    try {
...
const runAutoBackup = async () => {
    try {
        ...
        if (String(autoBackup) !== 'true' || !backupPath)
            return;
        ...
        const safeSettings = Object.fromEntries(Object.entries(allSettings || {}).filter(([key]) => !SENSITIVE_SETTINGS_KEYS.includes(key)));
        const payload = {
            version: '1.0.0',
