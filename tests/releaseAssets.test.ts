...
describe('release asset allowlist', () => {
  ...
  it('rebuilds and verifies better-sqlite3 for Electron before packaging', () => {
    ...
    expect(updaterOrchestrator).toContain('runtimeRoot = createUpdaterRuntimeRoot(projectRoot)')
    expect(updaterOrchestrator).toContain("stageCandidateFixture(oldFixture, runtimeRoot, 'old')")
    expect(updaterOrchestrator).toContain("stageCandidateFixture(newFixture, runtimeRoot, 'new')")
    expect(updaterOrchestrator).toContain('sha256FileSync(sourcePath) !== sha256FileSync(destinationPath)')
    expect(updaterOrchestrator).toContain("const manifestPath = path.join(runtimeRoot, 'fixture-manifest.json')")
    expect(updaterOrchestrator).toContain('removeUpdaterRuntimeRoot(runtimeRoot, projectRoot)')
    expect(updaterOrchestrator).toContain('removeUpdaterPlaywrightOutputDirectory(projectRoot)')
    expect(updaterOrchestrator).toContain('const cleanupErrors: unknown[] = []')
    expect(updaterOrchestrator).toContain('Disposable updater cleanup failed (${cleanupErrors.length} bounded errors)')
