...
describe('release asset allowlist', () => {
  ...
  it('rebuilds and verifies better-sqlite3 for Electron before packaging', () => {
    ...
    expect(updaterOrchestrator).toContain("const runtimePrefix = 'windows-updater-e2e-runtime-'")
    expect(updaterOrchestrator).toContain('runtimeRoot = createRuntimeRoot()')
    ...
    expect(updaterOrchestrator).toContain('fs.rmSync(runtimeRoot, { recursive: true, force: true })')
