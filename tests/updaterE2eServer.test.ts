...
function createArtifacts(): UpdaterServerArtifacts {
  ...
  const oldSetupPath = path.join(root, 'MindDiary-Setup-1.17.1.exe');
  const oldBlockmapPath = `${oldSetupPath}.blockmap`;
  const newSetupPath = path.join(root, 'MindDiary-Setup-1.17.2.exe');
  ...
  return {
    ...
    newSetupPath,
    newBlockmapPath,
    oldLatestPath: writeLatest('1.17.1', oldSetupPath, oldBytes),
    newLatestPath: writeLatest('1.17.2', newSetupPath, newBytes),
...
describe('loopback updater server', () => {
  it('serves only allowlisted metadata and files over IPv4 loopback', async () => {
    ...
    try {
      ...
      expect(await requestStatus(port, 'GET', {}, '/%2e%2e%2fsecret')).toBe(403);
      ...
      expect(await requestStatus(port, 'GET', { Authorization: 'Basic fixture-value' })).toBe(403);
      expect(await requestStatus(port, 'GET', { Cookie: 'fixture=value' })).toBe(403);
  ...
  it('produces an independently invalid checksum metadata case', () => {
    ...
    expect(metadata.badChecksum.version).toBe('1.17.2');
